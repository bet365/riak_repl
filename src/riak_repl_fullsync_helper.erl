...
make_keylist(Pid, Partition, Filename, FullsyncObjectFilter) ->
    riak_core_gen_server:call(Pid, {make_keylist, Partition, Filename, FullsyncObjectFilter}, ?LONG_TIMEOUT).
...
handle_call({make_keylist, Partition, Filename, FullsyncObjectFilter}, From, State) ->
    Ref = make_ref(),
                            ...
                            receive
                                {FoldRef, {Self, _}} ->
                                    ...
                                    riak_core_gen_server:cast(Self,
                                                              {kl_finish, 0});
                                {FoldRef, {Self, _Count, Total, _FullsyncObjectFilter}} ->
                                    riak_core_gen_server:cast(Self,
                                                              {kl_finish, Total});
...
keylist_fold({B,Key}=K, V, {MPid, Count, Total, FullsyncObjectFilter}) ->
    try
        ...
        check_keylist_ack({Count, MPid, Total, FullsyncObjectFilter}, Filtered)
    catch _:_ ->
            {MPid, Count, Total, FullsyncObjectFilter}
    end;
...
check_keylist_ack({Count, MPid, Total1, FullsyncObjectFilter}, Filtered) ->
    ...
    case Count of
        100 ->
            %% send keylist_ack to "self" every 100 key/value hashes
            ok = riak_core_gen_server:call(MPid, keylist_ack, infinity),
            {MPid, 0, Total2, FullsyncObjectFilter};
        _ ->
            {MPid, Count+1, Total2, FullsyncObjectFilter}
    end.
