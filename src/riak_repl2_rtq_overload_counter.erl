...
name(Id) ->
    ...
    binary_to_atom(<<"riak_repl2_rtq_overload_counter_", IdBin/binary>>, latin1).
...
start_link(Id) ->
    ...
    start_link(Id, [{report_interval, SendInterval}]).
...
stop(Id) ->
    gen_server:cast(name(Id), stop).

%% @private
drop(Id) ->
    gen_server:cast(name(Id), drop).
...
init([Id, Options]) ->
    ...
    {ok, #state{id = Id, interval = Report}}.
...
handle_info(report_drops, State) ->
    ...
    riak_repl2_rtq:report_drops(State#state.id, State#state.drops),
